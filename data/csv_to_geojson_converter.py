...
template = \
   ...
   { "type": "Feature",
       "geometry" : {
           "type": "Point",
           "coordinates": [%s,%s]},
       "properties": { "id" : "%s", "start_time" : "%s", "end_time": "%s", "bike_type" : "%s", "user_type": "%s", "start_lon": "%s", "start_lat": "%s", "end_lon": "%s", "end_lat": "%s", "start_station": "%s", "end_station": "%s"}
    },
   '''
...
for row in rawData:
   ...
   if iter >= 2 and (len(row[10]) != 0 or len(row[11]) != 0) and (row[2][0:10] == "2023-03-06" or row[2][0:10] == "2023-03-07" or row[2][0:10] == "2023-03-08" or row[2][0:10] == "2023-03-09" or row[2][0:10] == "2023-03-10" or row[2][0:10] == "2023-03-11" or row[2][0:10] == "2023-03-12") and row[1][0:13] == "electric_bike":
      ...
      output += template % (start_lng, start_lat, id, start_time, end_time, bike_type, user_type, start_lng, start_lat, end_lng, end_lat, start_station, end_station)
      #if i > 99: break
...
outFileHandle = open("data/230303citibike.geojson", "w")
